...
export class FrequencesTableComponent implements OnInit, AfterViewInit {
  ...
  reloadData() {
    ...
    this.categorieProduitService.getListItems().subscribe((categories: CategorieProduit[]) => {
      ...
      this.categoriesFiltered = categories.filter(c =>
        FREQUENCES_CATEGORIES.has((c.id))
      );
    });
    ...
    this.produitService.getListItems().subscribe((produits: Produit[]) => {
      this.produits = produits.filter(p => FREQUENCES_CATEGORIES.has(p.categorieProduit));
    });


    this.ficheTechniquesService.getFicheTechniques().subscribe((response: FicheTechniques[] = []) => {
      const byFreqCats = response.filter(f => FREQUENCES_CATEGORIES.has((f.categorie_produit)));

      this.ficheTechniques.data = byFreqCats.sort((a, b) => (b.id) - (a.id));

      this.ficheTechniques.filter = JSON.stringify(this.filterValues || {});
      if (this.ficheTechniques.paginator) {
        this.ficheTechniques.paginator.firstPage();
      }
    });
